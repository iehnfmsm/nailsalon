class Reservation < ApplicationRecord
  ...
  def date_six_month_end
    errors.add(:date, "は6ヶ月以降の日付は選択できません") if (Date.current >> 6) < date
  end
  ...
  def self.check_reservation_day(day)
    ...
    elsif (Date.current >> 6) < day
      return "6ヶ月以降の日付は選択できません。正しい日付を選択してください。"
    end
  end



  def self.reservations_after_six_month
    # 今日から6ヶ月先までのデータを取得
